...
capture.output(print(summary(indval, indvalcomp = TRUE)), file = "../indicator_species_analysis.txt")
...
ASVs_performes = core_members(order.perciformes, detection=0.01, prevalence = 0.1)
ASVs_blenniiformes = core_members(order.blenniiformes, detection=0.01, prevalence = 0.1)
ASVs_labriformes = core_members(order.labriformes, detection=0.01, prevalence = 0.1)
ASVs_centrarchiformes = core_members(order.centrarchiformes, detection=0.01, prevalence = 0.1)

ASVs_centrarchiformes

ggVennDiagram_plot = ggVennDiagram(list(ASVs_performes, ASVs_blenniiformes, ASVs_labriformes, ASVs_centrarchiformes), set_size = 6, category.names = c("P","B", "L", "C") )+scale_fill_gradient(low= "aliceblue", high = "lightskyblue1")

ggVennDiagram_plot

ggsave("../Core_Microbiome_Analysis.png", plot = ggVennDiagram_plot, width = 10, height = 5, dpi = 300)
...
Labriformes_plot = Labriformes_table %>% subset(!is.na(lfc_p_orderLabriformes)) |>
...
Blenniiformes_plot= Blenniiformes_table %>% subset(!is.na(lfc_p_orderBlenniiformes)) |>
...
Centrarchiformes_plot = Centrarchiformes_table %>% subset(!is.na(lfc_p_orderCentrarchiformes)) |> ggplot(aes(taxon,lfc_p_orderCentrarchiformes)) +
  geom_col() +
  coord_flip() +
   ggtitle("Centrarchiformes DESeq Result")

writexl::write_xlsx(Centrarchiformes_table,'../Centrarchiformes_ANCOMBC.xlsx')
writexl::write_xlsx(Blenniiformes_table,'../Blenniiformes_ANCOMBC.xlsx')
writexl::write_xlsx(Labriformes_table,'../Labriformes_ANCOMBC.xlsx')

ggsave("../Centrarchiformes_ANCOMBC_plot.png", plot = Centrarchiformes_plot, width = 10, height = 6, dpi = 300)
ggsave("../Labriformes_ANCOMBC_plot.png", plot = Labriformes_plot, width = 10, height = 6, dpi = 300)
ggsave("../Blenniiformes_ANCOMBC_plot.png", plot = Blenniiformes_plot, width = 10, height = 6, dpi = 300)

Blenniiformes_table
Centrarchiformes_table
Labriformes_table


```

```{r Saved-core-microbiome}
